...
public class Configurator_PLRS extends PApplet {
    ...
    if (serial.available() > 0) {
      if (serial.readChar() == 0x55) {
        ...
        switch (RXData[0]) {
          ...
          int a= PApplet.parseInt((RXData[2]<<8)|RXData[3]);
          if (a <= 125) {
            a = 0;
          } else if (a <= 250) {
            a = 1;
...
public void controlEvent(ControlEvent theEvent) {
  ...
  if (theEvent.getController().getId() == 1) {
    ...
    serial = new Serial(this, Serial.list()[selectedPortNum], 4800, 'E', 8, 1.0f);
    portSelected = true;
...
public void Wrire_All() {
  if (ConFlag) {
    ConFlag = false;
    byte[] TXData = {(byte)0x55, (byte)0xDB, 0, 0, 0, 0, 0, 0, (byte)0x0A};

    int ia = PApplet.parseInt(cp5.getController("Power").getValue());
    TXData[2]=(byte)ia;

    ia= PApplet.parseInt(cp5.getController("Bandwith").getValue());
    if (ia <= 0) {
      ia = 125;
    } else if (ia <= 1) {
      ia = 250;
    ...
    ia = PApplet.parseInt(cp5.getController("SPREADING FACTOR").getValue())+7;
    TXData[5]=(byte)ia;
    ia = PApplet.parseInt(cp5.getController("Coding Rate").getValue())+5;
    TXData[6]=(byte)ia;
...
public void InitSlider() {
  ...
  public void settings() {  size(380, 280); }
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "Configurator_PLRS" };
    if (passedArgs != null) {
